...
应用在菜单栏显示一个**单色模板图标**（自动适配亮/暗菜单栏的黑白）。托盘菜单实时显示状态，并提供 **显示/隐藏桌宠**、**总在最前**、**开机自启**、**隐藏 Dock 图标**、**退出** 等选项。*隐藏 Dock 图标* 会把应用切换为「仅菜单栏」的附属应用，且选择会被记住、重启后依然生效。
...
**如何退出 `waiting`：** 没有「批准通过」的 Hook，且被批准的长耗时工具在结束前不写任何记录，因此 `waiting` 会在以下任一情况清除：(a) Transcript 有新进展（工作已继续），或 (b) 会话文件被「触碰」——也就是你**打开了**需要确认的那个对话（要批准本来就得打开它）。当你离开、还没去看时，它会**一直**显示 `waiting` 以免漏掉；没有固定超时。

> **关于 `waiting` 与桌面 App：** 弹窗本身依赖 `PermissionRequest` / `Notification`。终端版 Claude Code 会发这些事件；部分桌面 App 版本可能不发，此时真实的权限弹窗会显示为 `running` 而非 `waiting`。之前的「挂起工具调用启发式」会把长耗时工具误判为「等待」，已移除。
